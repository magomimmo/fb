;;; ****************************** NOTES ******************************
;;;
;;; The fb/profiles.clj file is used for keeping the developer
...
;;; view of the lib.  The fb/profiles.clj should never contain
;;; any configuration for the :user profile. The content of
;;; fb/profiles.clj will be merge with the content from the
;;; ~/.lein/profiles.clj and fb/project.clj. Keep in mind that
;;; any in case of conflicts, the fb/profiles.clj takes
;;; precedence over the fb/project.clj which, in turn, takes
...
{:dev { ;; Add the out dir to the dirs to be cleaned by the lein clean
       ...
       :cljx {:builds [{:source-paths ["test/cljx"]    ;; cljx source dir
                        :output-path "target/test/clj" ;; clj output dir
                        :rules :clj} ;; clj generation rules

                       {:source-paths ["test/cljx"] ;; cljx source dir
                        :output-path "target/test/cljs" ;; cljs output dir
